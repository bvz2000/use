...
- **branch** - (Required). 
  ...
  - This name **MAY NOT INCLUDE** a - (dash) character. Underscores are ok.
...
Some very minor steps have been taken to provide a modest amount of security. The use python scripts can be set to refuse to run if they are not owned by root and not only writable by root (to prevent tampering with the source code). Use package files themselves can be set to only be processed if they have the same limitations (owned by root and only writable by root) to prevent someone from injecting another command into the use system without the end user being aware of it. 
...
These security precautions are generally used to prevent casual tampering with the use system, and are NOT exhaustive security measures meant to prevent any and all exploits of the use system. By default they are turned off, but they can be enabled by editing a few global variables at the top of the `permissions.py` file (see the note about their importance below).

It is not advisable to ever run a use command as root (or via sudo) regardless of whether these security features are enabled or not. Use commands, by default, have the ability to run arbitrary shell commands. If someone managed to hijack the use system, and you ran a use command as root, you are effectively running arbitrary commands as root (of course this also applies to ANY script on your system that you run as root so this does not present a brand new or novel threat). If the ability to run arbitrary commands (whether as root or as an unprivileged user) seems too much of a security hole, there is a setting within the code (in `permissions.py`) named "ALLOW_ARBITRARY_COMMANDS" that can be set to **False** which will prevent the system from running arbitrary commands. If you set this flag to false, the use system will only be able to set environmental variables and aliases. But even this mechanism could be an avenue for a security threat.

That said, let's be frank for a moment. If someone has gained access to your system and modified it such that the use system is now a vector for malware, you have much bigger issues to deal with. There is little that the use system can do that someone who already has access to your system could not do manually. And if they can modify the use system such that it threatens your system, they will certainly be modifying other tools that have an even bigger impact. Still, the use system *could* be used to trick you into running malware under your user account.

You should set these security settings to match your level of comfort, but always keeping in mind that they are threat mitigation and not threat elimination.

By default these security settings are disabled. If you wish to enable them, the source code contains several constants at the top of the file `permissions.py` that enables and disables these security features. You may set them to True or False to control how permission validations are performed.
